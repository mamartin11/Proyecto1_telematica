...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        printf("Recibida solicitud del cliente: %s\n", buffer);

        // Procesar la solicitud aquí...

        // Envía una confirmación al cliente
        char *confirmation = "Solicitud recibida y procesada con éxito.";
        sendto(sockfd, confirmation, strlen(confirmation), 0, (struct sockaddr*)&client_addr, sizeof(client_addr));
    }
